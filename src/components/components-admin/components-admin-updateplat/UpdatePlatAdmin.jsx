import "./UpdatePlatAdmin.scss";
import { PlatContext } from "../../../utils/context/PlatContext";
import { useContext, useState } from "react";
import { useNavigate, useParams } from "react-router-dom";

function UpdatePlatAdmin() {
  const { id } = useParams();
  const { listOfPlats, updPlat } = useContext(PlatContext);
  const platToUpdate = listOfPlats.find((p) => p.id.toString() === id);
  console.log(platToUpdate);
  const navigate = useNavigate();

  const [nomPlat, setNomPlat] = useState(platToUpdate.nom);
  const [descriptionPlat, setDescriptionPlat] = useState(
    platToUpdate.description
  );
  const [urlPhotoPlat, setUrlPhotoPlat] = useState(platToUpdate.photo);
  const [paysPlat, setPaysPlat] = useState(platToUpdate.pays);
  const [categoriePlat, setCategoriePlat] = useState(platToUpdate.categorie);
  const [prixPlat, setPrixPlat] = useState(platToUpdate.prix);
  ...
  return (
    <div className="updateplat-content">
...
export default UpdatePlatAdmin;
